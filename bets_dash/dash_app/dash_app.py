...
def race_types(selected_race_index):
    ...
    race_id = selected_race_index+1
    selected_race_obj = Race.objects.get(id=race_id)
    race_types = [{"label": "Qualification", "value": "quali"}, {"label": "Race", "value": "race"}]
    if selected_race_obj.is_sprint:
        race_types.append({"label": "Sprint", "value": "sprint"})

    return [
        dropdown_race_type.render(app=app, race_types=race_types)
    ]
...
def player_bets(selected_race_index, racetype, player):
    ...
    selected_race_obj = Race.objects.get(id=race_id)
    # todo: do this for sprint and quali
    player_bets = RaceBet.objects.filter(race=selected_race_obj, player=Player.objects.get(nickname=player))
...
def player_bets(selected_race_index, racetype, player):
    ...
    selected_race_obj = Race.objects.get(id=race_id)
    # todo: do this for sprint and quali
    player_points = PlayerPoints.objects.get(race=selected_race_obj, player=Player.objects.get(nickname=player)).points
