...
from bets_input.models import Player, Race, Driver, RaceBet
...
class F1DriversForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.form_action = "bets_input/"
...
class DriverExtrasForm(forms.Form):
    ...
    def __init__(self, curr_player, curr_race, race_type, *args, **kwargs):
        ...
        # 1. get current user's three bets concerning dnf
        curr_dnf_bets = RaceBet.objects.filter(player=curr_player, race=curr_race, dnf=True)
        # 2. set initial values equal to corresponding drivers' ids
        if len(curr_dnf_bets) >= 1:
            self.fields['dnf_select_1'].initial=curr_dnf_bets[0].driver.id
        if len(curr_dnf_bets) >= 2:
            self.fields['dnf_select_2'].initial=curr_dnf_bets[1].driver.id
        if len(curr_dnf_bets) >= 3:
            self.fields['dnf_select_3'].initial=curr_dnf_bets[2].driver.id
        # same for dotd and fastest_lap
        curr_dotd_bet = RaceBet.objects.get(player=curr_player, race=curr_race, dotd=True)
        self.fields['dotd_select'].initial=curr_dotd_bet.driver.id
        curr_fastest_lap_bet = RaceBet.objects.get(player=curr_player, race=curr_race, fastest_lap=True)
        self.fields['fastest_lap_select'].initial=curr_fastest_lap_bet.driver.id
